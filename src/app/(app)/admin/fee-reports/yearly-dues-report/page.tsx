...
import { getYearlyDuesReportDataAction } from './actions';
import Link from 'next/link';

const availableYears = [new Date().getFullYear(), new Date().getFullYear() - 1, new Date().getFullYear() - 2, new Date().getFullYear() - 3];
const ITEMS_PER_PAGE = 10;

type StudentWithDues = Student & { dueAmount: number, class?: { name: string, division: string } };

export default function YearlyDuesReportPage() {
    ...
    const loadReportData = useCallback(async () => {
        ...
        const result = await getYearlyDuesReportDataAction({
            adminUserId,
            filters: {
        ...
        setIsLoading(false);
    }, [toast, classFilter, yearFilter, searchTerm, classes.length]);
    ...
    const handleDownloadCsv = () => {
        ...
        link.setAttribute("download", `yearly_dues_report_${yearFilter}.csv`);
    ...
    return (
        <div className="flex flex-col gap-6">
            <PageHeader 
                title="Yearly Wise Dues Report"
                description="View students with outstanding fee dues for a selected academic year."
            ...
            <Card>
                ...
                <CardContent className="flex flex-wrap gap-4 items-end">
                    <Input placeholder="Search by student or father name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="max-w-sm"/>
                 ...
                 <CardContent>
                            ...
                            <TableBody>
                                {paginatedRecords.map(d => (
                                    <TableRow key={d.id}>
                                        ...
                                        <TableCell>{d.class?.name}</TableCell>
                                        <TableCell>{d.class?.division}</TableCell>
                    ...
                    {!isLoading && reportData.length === 0 && <p className="text-center text-muted-foreground py-4">No students with dues found for the selected criteria.</p>}
                 </CardContent>
                 {totalPages > 1 && (
