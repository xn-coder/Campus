...
import type { StudentFeePayment, Student, ClassData, Concession, PaymentMethod, Installment } from '@/types';
...
import { getInstallmentwiseFeeReportDataAction } from './actions';
...
export default function InstallmentwiseDailyCollectionReportPage() {
    ...
    const loadReportData = useCallback(async () => {
        ...
        const result = await getInstallmentwiseFeeReportDataAction({
            adminUserId,
            filters: {
                paymentStatus: paymentStatusFilter,
                classId: classFilter === 'all' ? undefined : classFilter,
                installmentId: installmentFilter === 'all' ? undefined : installmentFilter,
        ...
        if (result.ok) {
            ...
            if (installments.length === 0 && result.installments) setInstallments(result.installments || []);
        } else {
            toast({ title: "Error", description: result.message || "Failed to load report data.", variant: "destructive" });
        }
        setIsLoading(false);
    }, [toast, classFilter, installmentFilter, searchTerm, paymentStatusFilter, classes.length, installments.length, startDate, endDate]);
    ...
    const handleDownloadCsv = () => {
        ...
        const headers = ["Student Name", "Roll No.", "Class", "Section", "Installment", "Total Amount", "Concession", "Paid Amount", "Due Amount", "Mode", "Notes", "Date & Time"];
        const csvRows = [
            headers.join(','),
            ...reportData.map(d => {
        ...
        link.setAttribute("download", `installmentwise_collection_report_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    return (
        <div className="flex flex-col gap-6">
            <PageHeader 
                title="Installment Wise Collection Report"
                description="View and filter fee payments by installment for a specific day."
            ...
            <Card>
                ...
                <CardContent className="flex flex-wrap gap-4 items-end">
                    ...
                    <Select value={installmentFilter} onValueChange={setInstallmentFilter}>
                        <SelectTrigger className="w-full sm:w-[180px]"><SelectValue placeholder="Filter by installment"/></SelectTrigger>
                                ...
                                {paginatedRecords.map(d => {
                                    ...
                                    return (
                                        <TableRow key={d.id}>
                                            ...
                                            <TableCell>{d.installment?.title}</TableCell>
