...
import type { ClassData, Installment } from '@/types';
...
export async function getInstallmentwiseFeeReportDataAction(params: { adminUserId: string, filters: ReportFilters }) {
    ...
    try {
        ...
        let query = supabase.from('student_fee_payments')
            .select('*, student:student_id(*, class:class_id(name, division)), fee_category:fee_category_id(name), concessions:student_fee_concessions(concession_amount), installment:installment_id(title)')
        ...
        const [classesRes, installmentsRes] = await Promise.all([
            ...
            supabase.from('installments').select('*').eq('school_id', schoolId)
        ]);
