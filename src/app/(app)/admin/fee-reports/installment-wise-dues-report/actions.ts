...
export async function getInstallmentWiseDuesReportDataAction(params: { adminUserId: string, filters: ReportFilters }) {
    ...
    try {
        ...
        const [classesRes, installmentsRes] = await Promise.all([
             supabase.from('classes').select('*').eq('school_id', schoolId),
             supabase.from('installments').select('*').eq('school_id', schoolId).order('last_date', { ascending: false })
        ]);
        ...
        let targetInstallmentId = filters.installmentId;
        if (!targetInstallmentId && installments.length > 0) {
            targetInstallmentId = installments[0].id;
        }

        if (!targetInstallmentId) {
            return { ok: true, reportData: [], classes, installments };
        }
        ...
        let feeQuery = supabase.from('student_fee_payments')
            ...
            .eq('installment_id', targetInstallmentId);
        ...
        const dueStudents = (feeData || []).reduce((acc, item) => {
            ...
            if (dueAmount > 0) {
                if (!acc[item.student_id]) acc[item.student_id] = 0;
