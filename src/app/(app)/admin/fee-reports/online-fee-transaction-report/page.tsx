...
import type { ClassData, PaymentMethod } from '@/types';
import { Loader2, Search, ArrowLeft, Download, ChevronLeft, ChevronRight } from 'lucide-react';
import { useState, useEffect, useCallback, useMemo } from 'react';
import { useToast } from "@/hooks/use-toast";
import { format, parseISO, isValid } from 'date-fns';
import { getOnlineFeeTransactionReportDataAction } from './actions';
import Link from 'next/link';

const ITEMS_PER_PAGE = 10;

export default function OnlineFeeTransactionReportPage() {
    ...
    const loadReportData = useCallback(async () => {
        ...
        const result = await getOnlineFeeTransactionReportDataAction({
            adminUserId,
            filters: {
        ...
        if (result.ok) {
            ...
            if (paymentMethods.length === 0 && result.paymentMethods) {
                setPaymentMethods(result.paymentMethods.filter(pm => pm.name.toLowerCase() !== 'cash') || []);
            }
        } else {
            toast({ title: "Error", description: result.message || "Failed to load report data.", variant: "destructive" });
        }
        setIsLoading(false);
    }, [toast, classFilter, paymentMethodFilter, paymentStatusFilter, searchTerm, classes.length, paymentMethods.length]);
    ...
    const handleDownloadCsv = () => {
        ...
        const headers = ["Student Name", "Class", "Paid Amount", "Mode", "Notes", "Date & Time"];
        const csvRows = [
            headers.join(','),
            ...reportData.map(d => {
                const row = [
        ...
        link.setAttribute("download", `online_transactions_report_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    return (
        <div className="flex flex-col gap-6">
            <PageHeader 
                title="Online Fee Transaction Report"
                description="Review all fee payments made through non-cash methods."
            ...
            <Card>
                ...
                <CardContent className="flex flex-wrap gap-4 items-end">
                    ...
                    <Select value={paymentMethodFilter} onValueChange={setPaymentMethodFilter}>
                        <SelectTrigger className="w-full sm:w-[180px]"><SelectValue placeholder="Payment Type"/></SelectTrigger>
                        <SelectContent><SelectItem value="all">All Online Methods</SelectItem>{paymentMethods.map(m => <SelectItem key={m.id} value={m.name}>{m.name}</SelectItem>)}</SelectContent>
                    </Select>
                 ...
                 <CardContent>
                    ...
                    {!isLoading && reportData.length === 0 && <p className="text-center text-muted-foreground py-4">No online transactions found for the selected criteria.</p>}
                 </CardContent>
                 {totalPages > 1 && (
