...
import type { StudentFeePayment, Student, ClassData, Concession, PaymentMethod, FeeCategory } from '@/types';
...
import { getHeadwiseFeeReportDataAction } from './actions';
...
export default function HeadwiseDailyCollectionReportPage() {
    ...
    const loadReportData = useCallback(async () => {
        ...
        const result = await getHeadwiseFeeReportDataAction({
            adminUserId,
            filters: {
                paymentStatus: paymentStatusFilter,
                classId: classFilter === 'all' ? undefined : classFilter,
                feeCategoryId: feeCategoryFilter === 'all' ? undefined : feeCategoryFilter,
        ...
        if (result.ok) {
            ...
            if (feeCategories.length === 0 && result.feeCategories) setFeeCategories(result.feeCategories || []);
        } else {
            toast({ title: "Error", description: result.message || "Failed to load report data.", variant: "destructive" });
        }
        setIsLoading(false);
    }, [toast, classFilter, startDate, endDate, searchTerm, paymentStatusFilter, feeCategoryFilter, classes.length, feeCategories.length]);
    ...
    const handleDownloadCsv = () => {
        ...
        const headers = ["Student Name", "Roll No.", "Class", "Section", "Fee Type", "Total Amount", "Concession", "Paid Amount", "Due Amount", "Mode", "Date & Time"];
        const csvRows = [
            headers.join(','),
            ...reportData.map(d => {
                ...
                const row = [
                    ...
                    `"${d.fee_category?.name || 'N/A'}"`,
                    d.assigned_amount.toFixed(2),
                    d.total_concession.toFixed(2),
        ...
        link.setAttribute("download", `headwise_collection_report_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    return (
        <div className="flex flex-col gap-6">
            <PageHeader 
                title="Headwise Collection Report"
                description="View and filter fee payments by fee category (head) for a specific date range."
                ...
                <CardContent className="flex flex-wrap gap-4 items-end">
                    ...
                    <Popover>
                        <PopoverTrigger asChild>
                             <Button id="start-date" variant={"outline"} className="w-full sm:w-auto justify-start text-left font-normal">
                    ...
                    <Popover>
                        <PopoverTrigger asChild>
                            <Button id="end-date" variant={"outline"} className="w-full sm:w-auto justify-start text-left font-normal">
                                <CalendarIcon className="mr-2 h-4 w-4" />
                    ...
                    <Select value={feeCategoryFilter} onValueChange={setFeeCategoryFilter}>
                        <SelectTrigger className="w-full sm:w-[180px]"><SelectValue placeholder="Filter by fee type"/></SelectTrigger>
                        <SelectContent><SelectItem value="all">All Fee Types</SelectItem>{feeCategories.map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent>
                    </Select>
                    ...
                    <Select value={paymentStatusFilter} onValueChange={(val) => setPaymentStatusFilter(val as any)}>
                        <SelectTrigger className="w-full sm:w-[180px]"><SelectValue placeholder="Payment Status"/></SelectTrigger>
                        <SelectContent>
