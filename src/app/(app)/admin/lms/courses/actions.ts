...
import { addYears, addMonths, isPast } from 'date-fns';
...
export async function createSchoolSubscriptionOrderAction(params: {
  courseId: string;
  schoolId: string;
  amount: number;
  userCount?: number;
  ...
  const { courseId, schoolId, amount, userCount, shouldExtend } = params;
  const isRazorpayEnabled = process.env.RAZORPAY_ENABLED === 'true';
  const supabase = createSupabaseServerClient();
  
  const { data: course, error: courseError } = await supabase.from('lms_courses').select('max_users_allowed, subscription_plan').eq('id', courseId).single();
  if (courseError || !course) return { ok: false, message: `DB error fetching course: ${courseError?.message}` };
  
  const initialSeats = amount > 0 ? (course.max_users_allowed || 0) : 0;
  const userCountToAdd = (userCount || 0) + initialSeats;

  if (!isRazorpayEnabled) {
      const { data: existingSub } = await supabase.from('lms_school_subscriptions').select('subscribed_users_count, end_date').eq('course_id', courseId).eq('school_id', schoolId).single();
      
      let newEndDate: string | null = null;
      if (shouldExtend) {
          const baseDate = existingSub?.end_date && !isPast(new Date(existingSub.end_date)) ? new Date(existingSub.end_date) : new Date();
          newEndDate = (course.subscription_plan === 'monthly' ? addMonths(baseDate, 1) : addYears(baseDate, 1)).toISOString();
      }
      ...
      const { error } = await supabase.from('lms_school_subscriptions').upsert({
          ...
          end_date: newEndDate || existingSub?.end_date
      }, { onConflict: 'course_id, school_id' });
  ...
  const options = {
      ...
      notes: {
          ...
          should_extend: shouldExtend || false
      },
  };
...
export async function verifyCoursePaymentAndEnrollAction(
  ...
  try {
      ...
      if (paymentType === 'school_subscription') {
          ...
          const shouldExtend = orderDetails.notes?.should_extend === 'true' || orderDetails.notes?.should_extend === true;


          if (!courseId || !schoolId) return { ok: false, message: "School subscription details missing."};

          const [existingSubRes, courseRes] = await Promise.all([
            supabase.from('lms_school_subscriptions').select('subscribed_users_count, end_date').eq('course_id', courseId).eq('school_id', schoolId).single(),
            supabase.from('lms_courses').select('subscription_plan').eq('id', courseId).single()
          ]);
          
          const existingSub = existingSubRes.data;
          const course = courseRes.data;

          if (courseRes.error || !course) return { ok: false, message: "Could not retrieve course details for subscription."};
