...
export default function AdminLmsReportsPage() {
    ...
    const handleDownloadCsv = () => {
        ...
        const headers = ["Course Title", "Enrolled Students", "Completed", "In Progress", "Not Started", "Enrolled Teachers", "Completion Rate (%)"];
        const csvContent = [
            headers.join(','),
            ...reportData.map(d => `"${d.title.replace(/"/g, '""')}",${d.studentEnrollmentCount},${d.completedCount},${d.inProgressCount},${d.notStartedCount},${d.teacherEnrollmentCount},${d.completionPercentage.toFixed(2)}`)
        ].join('\n');
                                            ...
                                            <TableCell className="font-medium">
                                                <div className="flex flex-col">
                                                    ...
                                                    <div className="flex items-center gap-2 mt-1">
                                                        <Progress value={course.completionPercentage} className="h-1.5 w-24" title={`${course.completionPercentage.toFixed(0)}% Complete`} />
                                                        <span className="text-xs text-muted-foreground">{course.completionPercentage.toFixed(0)}%</span>
                                                    </div>
                                                </div>
                                            </TableCell>
                        ...
                        {isLoading ? <div className="flex justify-center items-center h-48"><Loader2 className="h-8 w-8 animate-spin" /></div> :
                         ...
                         <ChartContainer config={chartConfig} className="min-h-[300px] w-full">
                            <BarChart accessibilityLayer data={chartData} layout="vertical" margin={{ left: 10, right: 10 }}>
                                ...
                                <Bar dataKey="count" layout="vertical" fill="var(--color-enrollments)" radius={4} />
                            </BarChart>
                         </ChartContainer>
                        }
                    </CardContent>
