...
function RecordPaymentForm({ schoolId }: { schoolId: string }) {
    ...
    const loadInitialData = useCallback(async () => {
        ...
        if (result.ok) {
            ...
            if (result.methods && result.methods.length > 0) {
                setPaymentMode(result.methods[0].name); 
            } else {
                setPaymentMode('Cash');
            }
        } else {
    ...
    const allPaymentModes = useMemo(() => {
        const modes = new Set(paymentMethods.map(m => m.name));
        modes.add('Cash');
...
function PaymentMethodsManager({ schoolId }: { schoolId: string }) {
    ...
    const handleFormSubmit = async (e: FormEvent) => {
        e.preventDefault();
        if(!name.trim() || name.trim().toLowerCase() === 'cash') {
            toast({ title: "Invalid Name", description: "'Cash' is a reserved name and cannot be used.", variant: "destructive" });
            return;
        }
        setIsLoading(true);
                    ...
                    <div className="space-y-2">
                         ...
                         <div className="max-h-60 overflow-y-auto space-y-2 pr-2">
                            {isLoading ? <Loader2 className="animate-spin" /> : (
                                <>
                                    <div className="flex justify-between items-center p-2 border rounded-md bg-muted/50 opacity-70">
                                        <span className="font-medium">Cash</span>
                                        <div className="flex gap-1">
                                            <Button variant="ghost" size="icon" className="h-7 w-7" disabled><Edit2 className="h-4 w-4" /></Button>
                                            <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" disabled><Trash2 className="h-4 w-4" /></Button>
                                        </div>
                                    </div>
                                    {methods.map(method => (
                                            ...
                                            <AlertDialogContent>
                                                <AlertDialogHeader>
                                                    ...
                                                    <AlertDialogDescription>
                                                        This will permanently delete the "{method.name}" payment method. This action cannot be undone if it's not being used in any transactions.
                                                    </AlertDialogDescription>
                                                </AlertDialogHeader>
                                                <AlertDialogFooter>
                                                    <AlertDialogCancel>Cancel</AlertDialogCancel>
                                                    <AlertDialogAction onClick={() => handleDelete(method.id)} className="bg-destructive hover:bg-destructive/90">
                                                        Delete
                                                    </AlertDialogAction>
                                                </AlertDialogFooter>
                                            </AlertDialogContent>
                            ...
                            {!isLoading && methods.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">No custom methods created.</p>}
                         </div>
                    </div>
                </div>
                <DialogFooter>
