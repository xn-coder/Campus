...
import { Loader2, Ban, UploadCloud, Trash2 } from 'lucide-react';
...
import { deactivateSelfAction } from '@/actions/userActions';
...
export default function SchoolDetailsPage() {
  ...
  const handleDeactivate = async () => {
    ...
    setIsSubmitting(true);
    const result = await deactivateSelfAction(userId);
    if (result.ok) {
      toast({ title: "Account Deactivated", description: "You will be logged out." });
              ...
              <div className="flex gap-2">
                ...
                <AlertDialog>
                    <AlertDialogTrigger asChild>
                        <Button variant="destructive" disabled={isSubmitting}><Trash2 className="mr-2 h-4 w-4"/> Deactivate My Account</Button>
                    </AlertDialogTrigger>
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            <AlertDialogTitle>Are you absolutely sure?</AlertDialogTitle>
                            <AlertDialogDescription>
                                This will deactivate your own admin account. You will be logged out and will not be able to log back in unless reactivated by a superadmin. This will not affect other users.
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
